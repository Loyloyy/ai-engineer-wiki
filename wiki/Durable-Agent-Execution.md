...
- **Temporal workflow resume replays completed activities instantly — only unfinished work re-runs.** When a process is killed mid-agent-run, Temporal does not re-execute prior LLM calls or tool calls. It returns the cached results instantly (completing in milliseconds) until it reaches the point where execution stopped, then continues from there. This makes mid-run failures nearly free to recover from. Prerequisite: tool calls must be wrapped as Temporal activities, not just LLM calls — otherwise resume only covers the conversation, not the tool-call state. — Samuel Colvin, Pydantic AI ("From Stateless Nightmares to Durable Agents", AIE Code Summit 2025), [https://www.youtube.com/watch?v=flf_IKnFYnE](https://www.youtube.com/watch?v=flf_IKnFYnE)
- **Durable execution without tool-call activities is a chocolate teapot.** OpenAI's Temporal integration (as of mid-2025) handles LLM calls as activities but not tool calls — meaning agents with active tools cannot properly resume. Full durability requires tool calls to be treated as first-class durable steps. — Samuel Colvin, Pydantic AI ("From Stateless Nightmares to Durable Agents", AIE Code Summit 2025), [https://www.youtube.com/watch?v=flf_IKnFYnE](https://www.youtube.com/watch?v=flf_IKnFYnE)
...
- Samuel Colvin, Pydantic AI, "From Stateless Nightmares to Durable Agents", AIE Code Summit 2025 — [https://www.youtube.com/watch?v=flf_IKnFYnE](https://www.youtube.com/watch?v=flf_IKnFYnE)
